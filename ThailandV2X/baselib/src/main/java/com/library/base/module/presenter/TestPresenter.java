...
//        mView.showDialog();
        Subscription mSubscribe = mModel.login(userName, password)
                .subscribe(new JesSubscribe<List<TestBean>>(mView , true ) {
                    @Override
                    public void _onSuccess(List<TestBean> result) {
...
//                        mView.dismissDialog();
    ...
    private void testCache() {
        mSubscriptions.add(RetrofitCache.load("test", mModel.login("aaaa", "bbbb"), true, true)
                .subscribe(new JesSubscribe<List<TestBean>>(mView ) {
                    @Override
                    public void _onSuccess(List<TestBean> response) {
                        KLog.d(response);
                    }
