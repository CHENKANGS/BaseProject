...
//	}
	 ...
	 * @return
	 */
	public static String checkPhoneStr(String phoneStr) {
	...
	public static boolean checkPhone(String phoneStr) {
		// 进行正则匹配是否密码格式
		Pattern p = Pattern
				.compile("^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|19[0-9]|14[57])[0-9]{8}$");
		Matcher m = p.matcher(phoneStr);
		if (null == phoneStr.trim() || "".equals(phoneStr.trim()) || "请输入手机号".equals(phoneStr.trim())) {
			return false;// 代表为空
		} else if (!m.matches()) {
			return false;// 为格式不正确
		}
