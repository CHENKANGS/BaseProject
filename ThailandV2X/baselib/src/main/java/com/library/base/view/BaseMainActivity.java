...
public class BaseMainActivity extends BaseKcActivity {
    ...
    private CommonRcvAdapter<TestBean> adapter;
    private List<TestBean> ks_injured_damages;
...
//        manager.setSmoothScrollbarEnabled(true);
        adapter = new CommonRcvAdapter<TestBean>(ks_injured_damages = ks_injured_damages == null ? new ArrayList<TestBean>() : ks_injured_damages) {
            @NonNull
            @Override
...
//                });
    ...
    private void httpGetZX(String appkey, String appsecret) {
        HttpRequest.getInstance(this).getHomeZX(appkey, appsecret, new ResponseCallback<List<TestBean>>(this) {
            @Override
            public void onSuccess(final List<TestBean> resultData) {
...
//                human_address_tv.setText(resultData.getQuickcase_injured_id()+"");
            ...
            @Override
            public void onFailure(Call<ResponseModel<List<TestBean>>> call, Throwable t) {
