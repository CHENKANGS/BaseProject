...
//				view.setImageResource(imgsId[position]);
				if (TYPE_IMAGE_BD.equals(image_List.get(position).getTypeLoc())){//加载本地
//					int resourceId = R.mipmap.wzcx_image;
					int resourceId = Integer.parseInt(image_List.get(position).getPath());
					Glide.with(ImagePagerActivity.this).load(resourceId).into(view);
				}else if (TYPE_IMAGE_XC.equals(image_List.get(position).getTypeLoc())){//相册图片
					showImage(view, "file://" + image_List.get(position).getPath());
				}else if (TYPE_IMAGE_FXPG.equals(image_List.get(position).getTypeLoc())){//风险评估
//					showImage(view, Constant.BASE_URL + "/" + image_List.get(position).getPath());
				}else {//加载网络
					showImage(view,  image_List.get(position).getPath());
